...
from webui.routes import admin, scan, llm, websocket, docset
from webui.routes.scan import enqueue_scan_task
from webui.jinja_env import templates
from webui.middleware.metrics import PrometheusMiddleware, create_metrics_endpoint
    ...
    query = text("""
        WITH doc_set_pages AS (
            SELECT 
                ...
                p.id as page_id,
                CASE 
                    WHEN position('/azure/' in p.url) > 0 THEN
                        split_part(
                            substring(p.url from position('/azure/' in p.url) + 7),
                            '/', 
                            1
                        )
                    ELSE NULL
        ...
        SELECT 
            ...
            ROUND(
                CASE 
                    WHEN total_pages > 0 THEN (biased_pages::float / total_pages::float * 100)
                    ELSE 0
                END,
                1
            ) as bias_percentage
