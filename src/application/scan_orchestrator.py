...
from src.infrastructure.crawler_service import CrawlerService
...
class ScanOrchestrator:
    ...
    def process_web_scan(self, url: str, scan_id: int, force_rescan: bool = False) -> bool:
        ...
        try:
            ...
            progress_service.start_phase(self.db, scan_id, 'crawling', {
            ...
            if not crawled_results:
                progress_service.report_error(self.db, scan_id, "No pages crawled")
            ...
            progress_service.complete_phase(self.db, scan_id, 'crawling', {
            ...
            progress_service.start_phase(self.db, scan_id, 'queuing', {
            ...
            queued_count = self._queue_web_documents(crawled_results, page_objs, scan_id, force_rescan)
            if queued_count == 0:
                progress_service.report_error(self.db, scan_id, "No documents queued for processing")
                self._mark_scan_error(scan, "No documents queued for processing")
                return False
                
            progress_service.complete_phase(self.db, scan_id, 'queuing', {
    ...
    def process_github_scan(self, url: str, scan_id: int, force_rescan: bool = False) -> bool:
        ...
        try:
            ...
            if not page_objs:
                progress_service.report_error(self.db, scan_id, "No files discovered from GitHub")
                return False
            ...
            queued_count = self._queue_github_documents(
                github_service, parsed_url, page_objs, scan_id, force_rescan
            )
            
            if queued_count == 0:
                progress_service.report_error(self.db, scan_id, "No GitHub documents queued for processing")
    ...
    def _crawl_pages(self, url: str, scan: Scan, scan_id: int) -> tuple:
        ...
        for page_url, html in crawled_results.items():
            ...
            progress_service.update_phase_progress(
                self.db, scan_id, 
